...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        //When m isn't equal to zero, the GCD is calculated
        if (!m.isZero()) {
            NaturalNumber remainder = n.divide(m);
            reduceToGCD(m, remainder);
            n.copyFrom(m);
            //fulfilling contract by clearing m
            m.clear();
        }
    }
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        //this will apply to numbers of any size
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        //declaring variables for specific values
        final NaturalNumber one = new NaturalNumber2(1);
        final NaturalNumber two = new NaturalNumber2(2);
        //declaring variables for the source (original) values of n and p
        NaturalNumber sourceN = new NaturalNumber2(n);
        NaturalNumber sourceP = new NaturalNumber2(p);

        if (sourceP.isZero()) {
            //any value to the zero power is changed to 1
            n.copyFrom(one);
        } else if (isEven(sourceP)) {
            //if p is even, perform the operation n^p/2 ^ 2
            p.divide(two);
            powerMod(n, p, m);
            n.power(2);
            //the operation result is copied into n
            NaturalNumber remainder = n.divide(m);
            ...
            powerMod(n, p, m);
            n.power(2);
            //in the case that p is odd, multiply it by another factor of n
            n.multiply(sourceN);
            NaturalNumber remainder = n.divide(m);
            n.copyFrom(remainder);
        }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        //creating copies of the parameters
        NaturalNumber sourceW = new NaturalNumber2(w);
        NaturalNumber sourceWtwo = new NaturalNumber2(w);
        NaturalNumber sourceNdecrement = new NaturalNumber2(n);
        sourceNdecrement.decrement();
        //creating variables to hold values of 1 and 2
        final NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber two = new NaturalNumber2(2);

        //determining w^2 mod n
        powerMod(sourceW, two, n);
        //determining w^(n-1) mod n
        powerMod(sourceWtwo, sourceNdecrement, n);

        //based on the boolean expression in the contract, return true or false
        return (sourceW.compareTo(one) == 0)
                || (sourceWtwo.compareTo(one) != 0);
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        //storing n decrement(n-1)
        NaturalNumber nDecrement = new NaturalNumber2(n);
        nDecrement.decrement();
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            /*
             * 2 and 3 are primes
             */
            ...
            //creating 50 witness candidates
            final int numberFIFTY = 50;
            //declaring variable to hold value of 2
            NaturalNumber two = new NaturalNumber2(2);
            int i = 0;
            while (i < numberFIFTY) {
                // declaring variable for what has been ran/performed
                NaturalNumber performed = randomNumber(nDecrement);
                /*
                 * if the number is less than 2 or greater than n decrement(n-1)
                 * create a new value
                 */
                while (performed.compareTo(two) < 0
                        || performed.compareTo(nDecrement) >= 0) {
                    performed = randomNumber(nDecrement);
                }

                // if n is composite, it is concluded to not be a prime
                if (isWitnessToCompositeness(performed, n)) {
                    isPrime = false;
                }
                i++;
            }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        //if the number is even, increment its value to make it odd
        if (isEven(n)) {
            n.increment();
        }
        // variable that holds a value of 2
        NaturalNumber two = new NaturalNumber2(2);
        /*
         * continuously add 2 to the odd number until it has very likely become
         * a prime
         */
        while (!isPrime2(n)) {
            n.add(two);
