...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
         ...
         */
        String substring2 = str2.substring(overlap);

        /*
         * This line added just to make the program compilable. Should be
         * replaced with appropriate return statement.
         */
        return str1 + substring2;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // if addition is possible, add is true
        boolean add = true;
        for (String s : strSet) {
            if (s.contains(str)) {
                //if not, add is false
        ...
        if (add) {
            /*
             * an if loop that deletes substring content within the string we
             * need to add
             */
            Set<String> temporary = strSet.newInstance();
            while (strSet.size() > 0) {
                String s = strSet.removeAny();
                if (!str.contains(s)) {
                    temporary.add(s);
                }
            }
            temporary.add(str);
            strSet.transferFrom(temporary);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            ...
            addToSetAvoidingSubstrings(set, str);
        }
